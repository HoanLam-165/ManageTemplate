...
export function TemplateEditor({ initialTemplate, onClose }: TemplateEditorProps) {
  ...
  const checkIsDirty = (): boolean => {
    ...
    return lastSavedStateRef.current !== currentState;
  };
  ...
  async function saveTemplateInternal(): Promise<boolean> {
    try {
      if (templateId) {
    ...
    <div style={{ height: "100vh", display: "flex", flexDirection: "column", background: "#ffffff", overflow: "hidden" }}>
      ...
      <header style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "0 20px", height: "54px", backgroundColor: "#ffffff", borderBottom: "1px solid #e2e8f0", zIndex: 30 }}>
        <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
          ...
          <input type="text" value={name} onChange={(e) => setName(e.target.value)} style={{ fontWeight: 600, fontSize: "15px", color: "#1e293b", border: "none", outline: "none", background: "transparent", width: "200px" }} placeholder="Tên Template" />
        </div>
        ...
        <div style={{ display: "flex", gap: "8px" }}>
          ...
          <button style={{ backgroundColor: "#2563eb", color: "white", fontWeight: 600, padding: "6px 18px", borderRadius: "6px", border: "none", cursor: "pointer", fontSize: "13px", boxShadow: "0 2px 4px rgba(37,99,235,0.2)" }} onClick={() => saveTemplateInternal()}>Save</button>
          <button style={{ backgroundColor: "#ffffff", border: "1px solid #cbd5e1", color: "#334155", padding: "6px 14px", borderRadius: "6px", cursor: "pointer", fontSize: "13px", fontWeight: 500 }} onClick={saveAsNew}>Save As New</button>
        </div>
      </header>
      ...
      <div style={{ flex: 1, display: "flex", overflow: "hidden", position: "relative" }}>
        ...
        <aside style={{ width: "290px", borderLeft: "1px solid #e2e8f0", backgroundColor: "#ffffff", display: "flex", flexDirection: "column", zIndex: 20, padding: "16px", overflowY: "auto" }} onClick={(e) => e.stopPropagation()}>
          {selectedElement ? (
            <div>
              ...
              {selectedElement.element_type === "Text" && (
                <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
                  <div>
                    <label style={{ fontSize: "11px", fontWeight: 600, color: "#64748b", textTransform: "uppercase" }}>Nội dung mẫu</label>
              ...
              {selectedElement.element_type === "Checkbox" && (
                <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
                  <input type="checkbox" id="chk-prop-tpl" checked={!!selectedElement.properties.checked} onChange={(e) => { saveSnapshot(); updateElement({ ...selectedElement, properties: { ...selectedElement.properties, checked: e.target.checked } }); }} />
                  <label htmlFor="chk-prop-tpl" style={{ fontSize: "13px", color: "#334155", cursor: "pointer" }}>Mặc định đánh dấu</label>
                </div>
              )}
            ...
            <div>
              <span style={{ fontWeight: 600, fontSize: "14px", color: "#1e293b", display: "block", marginBottom: "16px" }}>⚙️ Thiết lập Template</span>
              <div style={{ marginBottom: "16px" }}>
                <label style={{ fontSize: "11px", fontWeight: 600, color: "#64748b", textTransform: "uppercase" }}>Mô tả Template</label>
                <textarea style={{ width: "100%", height: "60px", padding: "6px 8px", borderRadius: "6px", border: "1px solid #e2e8f0", fontSize: "12px", marginTop: "4px", boxSizing: "border-box", resize: "vertical" }} value={description} onChange={(e) => setDescription(e.target.value)} placeholder="Nhập mô tả cho biểu mẫu..." />
              </div>
              <div style={{ marginBottom: "16px" }}>
              ...
              <div style={{ marginBottom: "16px" }}>
                ...
                <p style={{ margin: "6px 0 0 0", fontSize: "13px", color: "#334155" }}>{content.elements.length} phần tử trong mẫu</p>
              </div>
            </div>
          )}
        </aside>
      ...
      {showCloseModal && (
        <div style={{ position: "fixed", inset: 0, backgroundColor: "rgba(0,0,0,0.4)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 9999 }}>
          <div style={{ width: "380px", backgroundColor: "#ffffff", padding: "24px", borderRadius: "12px", boxShadow: "0 20px 25px -5px rgba(0,0,0,0.2)" }}>
            ...
            <p style={{ margin: "0 0 20px 0", fontSize: "13px", color: "#64748b", lineHeight: "1.5" }}>Mẫu biểu của bạn có các chỉnh sửa chưa được lưu lại. Bạn có muốn lưu trước khi đóng không?</p>
            <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
              <button style={{ width: "100%", padding: "10px", backgroundColor: "#2563eb", color: "#ffffff", border: "none", borderRadius: "8px", fontWeight: 600, cursor: "pointer", fontSize: "13px" }} onClick={async () => {
                const saved = await saveTemplateInternal();
                if (saved) onClose();
              }}>💾 Lưu và thoát</button>
